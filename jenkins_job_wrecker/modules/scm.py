...
class Scm(jenkins_job_wrecker.modules.base.Base):
    ...
    def gen_yml(self, yml_parent, data):
        ...
        if scm_tag in self.registry.registry[self.component]:
            ...
            return
        if (
            scm_class is not None
            and scm_class in self.registry.registry[self.component]
        ):
        ...
        if child.tag == "configVersion":
            ...
            if len(list(child)) != 1:
                ...
                raise NotImplementedError(
                    "%s not supported with %i "
                    "children" % (child.tag, len(list(child)))
                )
            ...
            if len(list(child)) != 0:
                raise NotImplementedError(
                    "%s not supported with %i children" % (child.tag, len(list(child)))
                )
                ...
                if (
                    ...
                    if len(list(extension)) != 1:
                        ...
                        raise NotImplementedError(
                            ...
                            % (extension.tag, len(list(extension)))
                        )
                    if extension[0].tag != "relativeTargetDir":
                        raise NotImplementedError(
                            "%s XML not supported" % extension[0].tag
                        )
                    git["basedir"] = extension[0].text
                elif (
                    extension.tag == "hudson.plugins.git.extensions.impl.CheckoutOption"
                ):
                    if len(list(extension)) != 1:
                        # expected <timeout>
                        raise NotImplementedError(
                    ...
                    if extension[0].tag != "timeout":
                        raise NotImplementedError(
                            "%s XML not supported" % child[0][0].tag
                        )
                    git["timeout"] = extension[0].text
                elif (
                    extension.tag == "hudson.plugins.git.extensions.impl.WipeWorkspace"
                ):
                    ...
                    extension.tag == "hudson.plugins.git.extensions.impl.CleanCheckout"
                ):
                    ...
                    paths = {
                        "includedRegions": "included-regions",
    ...
    for child in top:
        if child.tag == "source":
            ...
            if browser_class == "hudson.plugins.mercurial.browser.BitBucket":
                ...
                raise NotImplementedError(
                    "%s is not yet supported by jenkins-job-builder." % browser_class
                )
    ...
    for child in top:
        if child.tag == "remote":
            ...
            if child.attrib["class"] == "hudson.scm.subversion.CheckoutUpdater":
                ...
                child.attrib["class"] == "hudson.scm.subversion.UpdateWithRevertUpdater"
            ):
                svn["workspaceupdater"] = "revertupdate"
            elif (
                child.attrib["class"] == "hudson.scm.subversion.UpdateWithCleanUpdater"
            ):
            ...
            svn["ignore-property-changes-on-directories"] = child.text == "true"
