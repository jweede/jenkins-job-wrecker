        ...
        if element.tag == "strategy":
            ...
            for subelement in element:
                if subelement.tag == "timeoutMinutes":
                    ...
                    timeout_inject["elastic-number-builds"] = int(subelement.text)
                elif subelement.tag == "timeoutMinutesElasticDefault":
                    timeout_inject["elastic-default-timeout"] = int(subelement.text)
                    ...
                    raise NotImplementedError(
                        "cannot handle " "XML %s" % subelement.tag
                    )
    ...
    for element in top:
        if element.tag == "deleteDirs":
            ...
            for subelement in element:
                ...
                if (
                    subelement.find("type") is not None
                    and subelement.find("pattern") is not None
                ):
...
def secretbuildwrapper(top, parent):
    ...
    for binding in top.find("bindings"):
        ...
        if (
            ...
            binding.tag == "org.jenkinsci.plugins.credentialsbinding.impl.StringBinding"
        ):
