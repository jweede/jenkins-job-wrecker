...
def junitresultarchiver(top, parent):
    ...
    for element in top:
        if element.tag == "testResults":
            ...
            junit_publisher["keep-long-stdio"] = element.text == "true"
        ...
        if element.tag == "configs":
            ...
            for sub in element:
                ...
                for config in sub:
                    if config.tag == "projects":
                    ...
                    elif config.tag == "condition" and config.text in [
                        "SUCCESS",
                        "UNSTABLE",
                        "FAILED_OR_BETTER",
                        "UNSTABLE_OR_BETTER",
                        ...
                        project["trigger-with-no-params"] = config.text == "true"
            ...
            for item in element:
                if item.tag == "name" and item.text in [
                    "SUCCESS",
                    "UNSTABLE",
                    "FAILURE",
                ]:
...
def mailer(top, parent):
    ...
    for element in top:
        if element.tag == "recipients":
            ...
            email_settings["send-to-individuals"] = element.text == "true"
        else:
            raise NotImplementedError("cannot handle " "email %s" % element.tag)
