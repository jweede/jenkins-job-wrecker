...
def parse_args(args):
    ...
    parser.add_argument("-f", "--filename", help="XML file to translate")
    parser.add_argument("-s", "--jenkins-server", help="Jenkins server to query")
    parser.add_argument("-n", "--name", help="Name of a job")
    parser.add_argument("-u", "--view", help="Name of a view")
    parser.add_argument(
        "-i", "--ignore", nargs="*", help="Ignore some jobs in conversion."
    )
    parser.add_argument(
        "-o",
        "--output-dir",
...
def main():
    ...
    if args.filename and not args.name and not args.view:
        log.critical(
            "Choose a job name (-n) or a view name (-u) for the job" " in this file."
        )
        exit(1)
    ...
    if args.jenkins_server:
        ...
        try:
            ...
            log.warning(
                "%s was not set as an environment variable to "
                "connect to Jenkins" % err
            )

        server = jenkins.Jenkins(
            args.jenkins_server, username=username, password=password
        )
